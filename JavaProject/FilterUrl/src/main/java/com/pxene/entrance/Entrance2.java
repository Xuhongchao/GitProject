...
public class Entrance2 {
	...
	public static void main(String[] args) {
		...
		List<String> noEnterUrl = new LoadNotEnterUrl().getNotEnterUrl(); // 拿待录入数据

		Map<List<String>, List<String>> map = new PreInputCheckCore().check(list, noEnterUrl); // 将已经在数据库中放到一个集合中，不在数据库的放到一个集合中
