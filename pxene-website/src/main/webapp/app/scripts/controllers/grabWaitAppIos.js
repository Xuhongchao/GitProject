define(['angular'], function () {
    return angular.module('dmpwebApp.controllers.grabWaitAppIosCtrl', [])
        .controller('grabWaitAppIosCtrl', ['$scope','$location','$http','$uibModal','uiGridConstants','gridService','$upload','$timeout','$compile','$templateCache', function ($scope,$location,$http,$uibModal,uiGridConstants,gridService,$upload,$timeout,$compile,$templateCache) {
            ...
            $scope.queryAppCategory = function(source){
                ...
                var param={appType:3,source:source.source_id !== "-1"?source.source_id:undefined};
            ...
            $scope.onFileSelect1 = function($files) {
                ...
                $upload.upload({
                    ...
                    //         $('#progressbg').remove()
                            ...
                            //     }
                            // })
                            if($location.path() == '/grabWaitAppIos'){
                                $scope.getPage("refush");
                                $scope.appsourcequery();
                            }
                            $("#export_input").val('');
